...
class CPlateLocate {
 public:
  ...
  int sobelFrtSearch(const cv::Mat& src, vector<cv::Rect_<float>>& outRects);
  ...
  //! 抗扭斜处理
  int deskew(const cv::Mat& src, const cv::Mat& src_b, vector<cv::RotatedRect>& inRects,
  ...
  //! Sobel运算
  //! 输入彩色图像，输出二值化图像
  int sobelOper(const cv::Mat& in, cv::Mat& out, int blurSize, int morphW, int morphH);
  ...
  //! 旋转操作
  bool rotation(cv::Mat& in, cv::Mat& out, const cv::Size rect_size, const cv::Point2f center,
                const double angle);
  ...
  //! 颜色定位法
  int plateColorLocate(cv::Mat src, vector<CPlate>& candPlates, int index = 0);

  //! Sobel定位法
  int plateSobelLocate(cv::Mat src, vector<CPlate>& candPlates, int index = 0);
  int sobelOperT(const cv::Mat& in, cv::Mat& out, int blurSize, int morphW, int morphH);

  //! Color搜索
  int colorSearch(const cv::Mat& src, const Color r, cv::Mat& out,
                  vector<cv::RotatedRect>& outRects, int index = 0);
  ...
  //! 车牌定位
  int plateLocate(cv::Mat, vector<cv::Mat>&, int = 0);

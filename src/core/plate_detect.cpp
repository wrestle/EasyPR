...
void CPlateDetect::LoadSVM(std::string s) { m_plateJudge->LoadModel(s.c_str()); }

int CPlateDetect::plateDetect(cv::Mat src, vector<CPlate>& resultVec,
...
int CPlateDetect::showResult(const cv::Mat& result) {
  cv::namedWindow("EasyPR", CV_WINDOW_AUTOSIZE);
    //cv class
  using cv::Mat;
  using cv::Rect;
    //cv function

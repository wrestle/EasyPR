...
CCharsIdentify::CCharsIdentify() {
  //cout << "start Create CCharsIdentify" << endl;
  m_predictSize = 10;
  m_path = "C:/git/EasyPR/EasyPR/resources/model/ann.xml";
  LoadModel();
 ...
 // std::cout << "CCharsidentify end!" << std::endl;
}
...
void CCharsIdentify::LoadModel() {
	...
	if (ann)
		ann->clear();
	ann = cv::Algorithm::load<cv::ml::ANN_MLP>(m_path);
	//std::cout << "LoadModel() end" << std::endl;
	//2.4.8
	//ann->load<cv::ml::ANN_MLP>(m_path.c_str(), "ann");
}

void CCharsIdentify::LoadModel(string s) {
	//std::cout << "In LoadModel(string s)" << std::endl;
  if (ann)
	ann->clear();
  ann = cv::Algorithm::load<cv::ml::ANN_MLP>(s);
  //std::cout << "LoadModel(string) end" << std::endl;
  //ann->load<cv::ml::ANN_MLP>(s.c_str());//, "ann");
}
...
string CCharsIdentify::charsIdentify(cv::Mat input) {
	//cv class
	using cv::Mat;
...
string CCharsIdentify::charsIdentify(cv::Mat input, bool isChinese, bool isSpeci) {
	//cv class
	using cv::Mat;

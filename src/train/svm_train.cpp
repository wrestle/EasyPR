...
void Svm::train(bool divide /* = true */, float divide_percentage /* = 0.7 */,
  ...
  if (out_svm_path == NULL) {
    out_svm_path = "C:/git/EasyPR/EasyPR/resources/model/svm.xml";
  }
  ...
  cv::Ptr<cv::ml::SVM> svm_sample = cv::ml::SVM::create();
  ...
  if (train) {
    ...
    if (!this->classes_.empty() && !this->trainingData_.empty()) {
      ...
      svm_sample->setType(cv::ml::SVM::C_SVC);
      // SVM_params.kernel_type = CvSVM::LINEAR; 
      // 线型，也就是无核
	  svm_sample->setKernel(cv::ml::SVM::RBF);
       // CvSVM::RBF 径向基函数，也就是高斯核
	  svm_sample->setDegree(0.1);
	  svm_sample->setGamma(1);
	  svm_sample->setCoef0(0.1);
	  svm_sample->setC(1.0);
	  svm_sample->setNu(0.1);
	  svm_sample->setP(0.1);
	  svm_sample->setTermCriteria(cv::TermCriteria(CV_TERMCRIT_ITER, 100000, 0.0001));
	  /*
	  ...
	  */
      ...
      try {
		  cv::Ptr<cv::ml::TrainData> svmTrainData = cv::ml::TrainData::create(this->trainingData_, cv::ml::ROW_SAMPLE, this->classes_, 
																				cv::Mat(), cv::Mat());
		  svm_sample->trainAuto(svmTrainData, 10,
      ...
      try {
		  svm_sample->load<cv::ml::SVM>("C:/git/EasyPR/EasyPR/resources/train/svm.xml", "svm");
  ...
  svm_sample->load<cv::ml::SVM>(out_svm_path, "svm");  // make sure svm model was loaded
  ...
  for (auto image : test_imgaes_) {
    ...
    Label predict = ((int)svm_sample->predict(out)) == 1 ? kForward : kInverse;
